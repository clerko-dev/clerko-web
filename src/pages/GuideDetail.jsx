...
export default function GuideDetail() {
  ...
  return (
    <>
      <SEO
        title={`${guide.title} — Guides — Clerko`}
        description={guide.excerpt}
      />

      <section className="max-w-6xl mx-auto px-4 pt-16 pb-6">
        <nav className="text-xs text-white/50 mb-3">
          <Link className="hover:opacity-80 underline underline-offset-4" to="/guides">
            Guides
          </Link>{" "}
        ...
        <motion.p {...fade(0.05)} className="text-zinc-400 mb-4 max-w-3xl">
          {guide.excerpt}
        </motion.p>

        <div className="text-xs text-white/50 flex gap-3 mb-8">
          ...
          <article className="prose prose-invert max-w-none">
            {sections.map((s) => (
              <section key={s.h} className="mb-8">
                <h2 id={s.h} className="scroll-mt-24 text-xl font-semibold mb-3">
                  {s.h}
                </h2>
                {s.p &&
                  s.p.map((para, i) => (
                ...
                {s.list && (
                  <ul className="list-disc pl-6 text-zinc-300 leading-relaxed space-y-1">
                    {s.list.map((li, i) => (
                      <li key={i}>{li}</li>
                    ))}
                  </ul>
        ...
        <div className="mt-12 rounded-2xl border border-white/10 bg-gradient-to-r from-white/[0.05] to-white/[0.02] p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Link to="/" className="px-4 py-2 rounded-xl border border-white/20 text-sm hover:border-white/40">
              Try free
            </Link>
            <Link to="/pricing" className="px-4 py-2 rounded-xl bg-white text-black text-sm hover:opacity-90">
              View pricing
            </Link>
          </div>
        </div>
